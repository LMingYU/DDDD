...
def pre_process2(x):
    ...
    l_images = transforms.functional.normalize(x, m, s)
    #l_images = torch.from_numpy(l_images)
    return l_images
...
for i in range(1):
   ...
   u_loader = DataLoader(
       u_train_dataset, shared_cfg["batch_size"]  # //2
       , drop_last=True,
   ...
   G=wrn_d_SVHN.WRN2(2, 6, None).to(device)
   Gbackl=wrn_d_SVHN.GeneratorResNetyx(3).to(device)
   Gbacku=wrn_d_SVHN.GeneratorResNetyx(3).to(device)
   D=wrn_d_SVHN.adversarialnet(128).to(device)
   C=wrn_d_SVHN.WRN_C(2, 6,transform_fn).to(device)
   C2 = wrn_d_SVHN.WRN_C(2, 6, transform_fn).to(device)

   optim_g=optim.Adam(G.parameters(), lr=3e-3,weight_decay=0.00001)
   adam_betas = (0.5, 0.999)
   optim_gl = optim.Adam(Gbackl.parameters(), lr=3e-3)
   optim_gu = optim.Adam(Gbacku.parameters(), lr=3e-3)
   optim_d=optim.SGD(D.parameters(), lr=3e-3,weight_decay=0.00001)
   optimizer_c =optim.Adam(C.parameters(), lr=3e-3,weight_decay=0.0005)
   optimizer_c2 = optim.Adam(C2.parameters(), lr=3e-3, weight_decay=0.0001)
   scheduler = lr_scheduler.ReduceLROnPlateau(optimizer_c, mode='max', factor=0.9, patience=1)
   ...
   for i in range(1):
    ...
    for l_data, u_data in zip(l_loader, u_loader):
     ...
     if iteration<0:
        ...
        cls_lossc2 = F.cross_entropy(outc2, labels, reduction='none',
                                   ignore_index=-1).mean()
        loss=cls_lossc2
        ...
        realPriImages=torch.tensor([item.cpu().detach().numpy() for item in realPriImages]).cuda()
        ...
        for lt,wt in zip(d[:len(l_labels)],w[:len(l_labels)]):
            d_lossw = d_lossw - (wt) * torch.log(lt)
            

        for ut,wt in zip(d[len(l_labels):len(d)],w[len(l_labels):len(w)]):
            d_lossw = d_lossw - (wt) * torch.log(1-ut)
        if iteration % 20 == 0:
        ...
        images2 = torch.cat([l_images_back,u_images_back],0)

        cycle_func=torch.nn.L1Loss()
        ...
        if (numofRealPriImages != 0):
         ...
         for tmp_images in gpriimages:
                for tmp_images2 in guimages:
                    tmp_res = torch.norm(tmp_images.view(1, -1) - tmp_images2.view(1, -1))
                    vu.append(tmp_res)
         vu = torch.tensor(vu)
         zu = torch.sum(vu)

         for i in range(len(vu)):
                vu[i]=torch.sigmoid(vu[i])
                pri_loss = pri_loss - torch.log(vu[i])
        ...
        g_loss=0
        for lt,wt in zip(d[:len(l_labels)],w[:len(l_labels)]):
            g_lossw = g_lossw - (wt) * torch.log(lt)
        for ut,wt in zip(d[len(l_labels):len(d)],w[len(l_labels):len(w)]):
            g_lossw = g_lossw - (wt) * torch.log(1-ut)
        if iteration % 20 == 0:
        ...
        cls_loss2 = F.cross_entropy(out2, labels[0:len(l_images_back)], reduction='none',
                                    ignore_index=-1).mean()
        cls_loss2=cls_loss2 * 0.0001
        ssl_loss2=F.softmax(out2u,dim=1)*F.log_softmax(out2u,dim=1)
        ...
        out_u=torch.nn.functional.softmax(out_u,dim=1)
        out_u2=torch.sort(out_u, dim=1, descending=True)[0]
